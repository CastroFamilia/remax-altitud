---
type: competitor-gap-findings
session_date: '2026-06-02'
focus_areas: ['serp analysis', 'competitor audit', 'topical gaps', 'content strategy']
target_regions: ['Pérez Zeledón', 'Dominical']
---
...
```mermaid
graph TD
    A[Competitor Content Hubs] --> B(What They Cover)
    A --> C(What They Miss: The Opportunity)
    
    B --> B1[Generic Expat Buying Steps]
    B --> B2[General Residency Info]
    B --> B3[Tourist-focused Blogs]
    
    C --> C1[ASADA Legal Water Verification]
    C --> C2[Plan Regulador & Uso de Suelo]
    C --> C3[Bioclimatic Mountain Construction]
    C --> C4[Ocean-View Height Restrictions]
    C --> C5[Micro-Community Infrastructure]
```
...
```mermaid
flowchart TD
    Start([1. Property Due Diligence]) --> WaterSource{2. Water Source Type?}
    WaterSource -->|AyA / ASADA| CheckLetter[3. Request Carta de Disponibilidad]
    WaterSource -->|Well / Spring| CheckConcession[3. Verify MINAE Concession]
    
    CheckLetter --> ValidLetter{4. Letter Valid & Current?}
    ValidLetter -->|No: Expired or Draft| Reject[5. RISK: Building Permit Will Be Denied]
    ValidLetter -->|Yes: Valid 6-12 Months| LegalBoard{5. ASADA Legally Constituted?}
    
    LegalBoard -->|No: Informal Board| Reject
    LegalBoard -->|Yes: Registered with AyA| Approved([6. WATER LEGALLY VERIFIED])
    
    CheckConcession --> HasConcession{4. Concession Registered? /Folio Real/}
    HasConcession -->|No| Reject
    HasConcession -->|Yes| Approved
```
