...
export function LifestyleQuestionnaire({ initialProperties, locale }: LifestyleQuestionnaireProps) {
  ...
  return (
    <section className="relative my-12 overflow-hidden rounded-2xl border border-brand-gold/30 bg-gradient-to-br from-brand-navy/95 to-brand-navy/90 p-8 text-white shadow-2xl md:p-12">
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        ...
        <div className="mb-8 text-center">
          <span className="inline-block rounded-full bg-brand-gold/20 px-4 py-1 text-[11px] font-bold tracking-wider text-brand-gold uppercase">
            {t("title")}
          </span>
        ...
        {step === 0 && (
          <div className="flex flex-col items-center text-center space-y-6 animate-fade-in">
            <div className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-md md:p-8">
              <h3 className="text-xl font-semibold text-brand-gold md:text-2xl">
                {t("welcome.heading")}
              </h3>
              <p className="mt-4 text-base leading-relaxed text-white/80 md:text-lg">
                {t("welcome.description")}
              </p>
              <p className="mt-4 text-sm font-medium text-brand-gold/90">
                {t("welcome.timeEstimate")}
              </p>
            </div>
            <button
              onClick={handleStart}
              className="group relative flex min-h-12 items-center rounded-xl bg-brand-gold px-8 py-3 font-semibold text-brand-navy shadow-lg transition-all duration-300 hover:scale-105 hover:bg-brand-gold-dark"
            >
              {t("welcome.start")}
              <svg
        ...
        {step >= 1 && step <= 4 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="h-1.5 w-full rounded-full bg-white/10">
              <div
                className="h-1.5 rounded-full bg-brand-gold transition-all duration-500 ease-out"
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-brand-gold md:text-xl">
                {t(`questions.q${step}.title`)}
              </h3>
              <p className="text-base text-white/90">{t(`questions.q${step}.subtitle`)}</p>
            </div>
              ...
              {(["A", "B", "C"] as Choice[]).map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-start text-left gap-4 rounded-xl border p-4 transition-all duration-200 backdrop-blur-sm cursor-pointer",
                      isSelected
                        ? "border-brand-gold bg-brand-gold/15 shadow-[0_0_15px_rgba(194,166,97,0.2)]"
                  ...
                  >
                    ...
                    <span className="text-sm font-medium leading-relaxed text-white/90 md:text-base">
            ...
            <div className="flex justify-between items-center pt-6 mt-8 border-t border-white/10">
              <button
                onClick={handleBack}
                className="flex items-center gap-2 rounded-lg border border-white/20 bg-transparent px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/10"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-lg px-6 py-2.5 text-sm font-bold shadow-md transition-all duration-200",
                  currentSelection
        ...
        {step === 5 && resultProfile && (
          <div className="space-y-8 animate-fade-in">
            {/* Vibe & Profile Card */}
            <div className="rounded-xl border border-brand-gold/30 bg-white/5 p-6 backdrop-blur-md md:p-8">
              ...
              <div className="mt-8 space-y-6">
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {getPlacesForProfile(resultProfile).map((place, idx) => (
                    <div
                      key={idx}
                      className="rounded-lg border border-white/5 bg-white/5 p-4 hover:border-white/10"
                    >
                      <h5 className="font-bold text-white text-base flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full bg-brand-gold" />
                        {place.name}
                      </h5>
                      <p className="mt-2 text-xs leading-relaxed text-white/70">{place.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4">
              <h4 className="text-center text-lg font-bold text-brand-gold uppercase tracking-wider">
            ...
            <div className="flex justify-center pt-6">
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-white/20 bg-transparent px-6 py-3 font-semibold text-white transition-colors hover:bg-white/10 cursor-pointer"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
