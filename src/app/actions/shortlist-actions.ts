...
export async function getShortlistPropertiesWithAgents(ids: string[]): Promise<any[]> {
  ...
  return rows.map((row) => ({
    ...
    lotSizeM2: row.properties.lotSizeM2,
    constructionM2: row.properties.constructionM2,
    zmtStatus: row.properties.zmtStatus ?? "titled",
    propertyType: row.properties.propertyType,
    listingType: row.properties.listingType ?? "Sale",
    status: row.properties.status,
    areaSlug: row.properties.areaSlug,
    subLocation: row.properties.subLocation ?? null,
    currency: row.properties.currency,
    apiRaw: row.properties.apiRaw as Record<string, unknown> | null,
    descriptionEn: row.properties.descriptionEn ?? "",
    descriptionEs: row.properties.descriptionEs ?? "",
    agent: row.agents
      ? {
          id: row.agents.id,
