...
const descriptionEn = `San Isidro de El General: A City That Has It All with Excellent Connectivity
...
const metadata = {
  h1En: "Barefoot Luxury & Mountain Living: Discover Pérez Zeledón, Costa Rica",
  h1Es: "Lujo Descalzo y Vida de Montaña: Descubra Pérez Zeledón, Costa Rica",
  seoTitleEn: "Affordable Properties for Sale in Pérez Zeledón | Barefoot Luxury Costa Rica",
  seoTitleEs: "Propiedades en Venta en Pérez Zeledón | Lujo Descalzo Costa Rica",
  seoDescriptionEn:
    "Explore real estate in Pérez Zeledón. Discover affordable barefoot luxury homes, off-grid mountain farms with private waterfalls, and properties near San Isidro de El General.",
  seoDescriptionEs:
    "Explore bienes raíces en Pérez Zeledón. Descubra casas de lujo, fincas de montaña autosostenibles con cascadas privadas y propiedades cerca de San Isidro.",
  elevation: "700m",
  climate: "Humid tropical / Fresco",
  nearestAirport: "San José (SJO) — 3.5 hours / Pérez Zeledón Airstrip",
  ...
  investmentContext: {
    ...
    rentalYieldEstimate: "4-6% for long-term rentals",
    marketHighlights: [
      "Growing expat community",
      "New hospital and university",
...
async function main() {
  console.log("Updating Pérez Zeledón description and metadata in database...");
  ...
  const result = await db
    ...
    .where(eq(areas.slug, "perez-zeledon"))
    .returning();
...
main().catch((err) => {
  console.error("Failed to seed Pérez Zeledón updates:", err);
  process.exit(1);
});
