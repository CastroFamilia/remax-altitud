...
async function main() {
  console.log("Updating Uvita description with [CARDINAL_MAP] and [CARDINAL_CARDS] tokens...");
  ...
  console.log("Database update success! Updated row:", JSON.stringify(result[0]?.slug));

  await client.end();
}

main().catch((err) => {
  console.error("Failed to update Uvita tokens:", err);
  process.exit(1);
});
