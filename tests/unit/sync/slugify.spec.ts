...
describe("slugify", () => {
  it("[P0] lowercases and hyphenates a plain English title", () => {
    // AC #3 — slug must be generated from titleEn
    expect(slugify("Mountain View Land")).toBe("mountain-view-land");
  });

  it("[P0] strips accented characters via NFD decomposition", () => {
  ...
  it("[P0] trims leading and trailing hyphens", () => {
    expect(slugify("--hello--")).toBe("hello");
  });

  it("[P0] replaces special characters with hyphens and collapses multiples", () => {
    // Task 9: non-alphanumeric → hyphen, collapse runs
    expect(slugify("lot (1) A")).toBe("lot-1-a");
  });

  it("[P1] appends a suffix when provided (for slug conflict resolution)", () => {
  ...
  it("[P1] returns an empty string for empty input", () => {
    // Edge case: no title available
    expect(slugify("")).toBe("");
  });

  it("[P1] handles a title consisting entirely of special characters", () => {
    // Should not produce a slug that starts/ends with hyphens
    expect(slugify("---")).toBe("");
  });

  it("[P2] handles Spanish characters correctly (ñ, ü, ó, etc.)", () => {
    expect(slugify("Niño de Oro")).toBe("nino-de-oro");
    expect(slugify("Müller")).toBe("muller");
  });

  it("[P2] handles very long titles by producing a valid slug", () => {
  ...
  it("[P3] preserves digits in slugs", () => {
